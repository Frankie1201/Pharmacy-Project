...
public class UpdMed2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter())
        {
            ...
            int r = MedicineQuery.update2(medId, supplierId);
            
            if(r==1)
            {
                out.print("Medicine record updated");
                request.getRequestDispatcher("medicineDisplay.jsp").include(request, response);
            }
            else
            {
                out.print("Error......Can not Update record");
                request.getRequestDispatcher("UpdMed.jsp").include(request, response);
            }
        } 
    }
