...
public class CreateStaff extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()){
            ...
            Staff s = new Staff(fullName,address,phone,email,password);
            
            //insert student into DB
            int r = StaffQuery.insert(s);
            
            if(r==1)
            {
                out.print("Pharmacist Record Saved"); 
                request.getRequestDispatcher("pharmacistDisplay.jsp").include(request, response);
            }//if ends
            else{
                out.print("Error......Can not save record");
                request.getRequestDispatcher("addphar.jsp").include(request, response);
            }//else ends
        }
    }
