...
public class UpdCust5 extends HttpServlet {
    ...
    String password;
        ...
        try (PrintWriter out = response.getWriter())
        {
            ...
            int r = CustomerQuery.update5(customerId, password);
            
            if(r==1)
            {
                out.print("Customer record updated");
                request.getRequestDispatcher("customerDisplay.jsp").include(request, response);
            }
            else
            {
                out.print("Error......Can not Update record");
                request.getRequestDispatcher("updCust.jsp").include(request, response);
            }
        }
    }
